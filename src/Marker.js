...
class Marker {
  constructor(lat, lon, text, altitude, previous, scene, _opts) {
    ...
    var point, previousPoint, markerMaterial, labelCanvas, labelTexture, labelMaterial;
    ...
    if (this.previous) {
      var materialSpline, materialSplineDotted, latdist, londist, startPoint, pointList = [], pointList2 = [], nextlat, nextlon, currentLat, currentLon, currentPoint, currentVert, update;
      ...
      for (var j = 0; j < _this.opts.lineSegments + 1; j++) {
        ...
        const sPoint = new THREE.Vector3(
          startPoint.x * 1.2,
          startPoint.y * 1.2,
          startPoint.z * 1.2
        );
        const sPoint2 = new THREE.Vector3(
          startPoint.x * 1.2,
          startPoint.y * 1.2,
          startPoint.z * 1.2
        );
      ...
      update = function () {
        ...
        for (var x = 0; x < _this.geometrySpline.vertices.length; x++) {
          ...
          const currentVert2 = _this.geometrySplineDotted.vertices[x];
          const currentPoint2 = utils.mapPoint(nextSpot2.lat, nextSpot2.lon);
  ...
  remove() {
    ...
    var update = (ref) => {
      ...
      if (x < ref.geometrySpline.vertices.length) {
        setTimeout(function () {
          update(ref);
        }, this.opts.drawTime / this.opts.lineSegments);
      } else {
